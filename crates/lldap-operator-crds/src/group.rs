...
#[kube(
    ...
    shortname = "llg",
    namespaced,
    status = "LldapGroupStatus",
    derive = "PartialEq",
    printcolumn = r#"{"name":"DisplayName","type":"string","jsonPath":".spec.displayName"}"#,
    printcolumn = r#"{"name":"GroupId","type":"integer","jsonPath":".status.groupId"}"#,
...
mod tests {
    ...
    fn spec_schema() -> serde_json::Value {
        let schema = serde_json::to_value(LldapGroup::crd()).expect("serialize CRD");
    ...
    fn crd_has_expected_group_version_kind() {
        let crd = LldapGroup::crd();
        assert_eq!(crd.spec.group, "lldap-operator.lukidoescode.com");
        assert_eq!(crd.spec.versions[0].name, "v1alpha1");
        assert_eq!(crd.spec.names.kind, "LldapGroup");
        assert_eq!(crd.spec.names.plural, "lldapgroups");
    }

    #[test]
    fn spec_round_trips_through_yaml() {
        let spec = LldapGroupSpec {
            display_name: "Engineering".to_owned(),
            attributes: None,
        };
        let yaml = serde_yaml_ng::to_string(&spec).expect("serialize");
        let back: LldapGroupSpec = serde_yaml_ng::from_str(&yaml).expect("deserialize");
        assert_eq!(spec, back);
    }
    ...
    fn spec_round_trips_with_attributes() {
        let spec = LldapGroupSpec {
            ...
            attributes: Some(vec![AttributeValue {
                name: "cost-center".to_owned(),
                value: vec!["CC-42".to_owned(), "CC-99".to_owned()],
            }]),
        };
        let yaml = serde_yaml_ng::to_string(&spec).expect("serialize");
        let back: LldapGroupSpec = serde_yaml_ng::from_str(&yaml).expect("deserialize");
    ...
    #[test]
    fn display_name_schema_has_correct_length_constraints() {
        let schema = spec_schema();
        assert_eq!(schema["properties"]["displayName"]["minLength"], 1);
        assert_eq!(schema["properties"]["displayName"]["maxLength"], 128);
    }

    #[test]
    fn crd_includes_instance_label_validation() {
        let yaml = serde_yaml_ng::to_string(&LldapGroup::crd()).expect("serialize");
    ...
    fn status_struct_defaults_to_all_none() {
        let status = LldapGroupStatus::default();
